...
USER = os.getenv("PROFILE_USERNAME", "devRko")
...
if days:
    ...
    d = today if by_date.get(str(today), 0) > 0 else today - timedelta(days=1)
...
# Dashboard SVG
svg = svg_start(1100, 370, "⚡ LIVE DEVELOPER COMMAND CENTER")
svg += card(30, 70, 245, 115, "Public Repositories", fmt(len(public_repos)), "visible portfolio repositories")
...
svg += card(160, 200, 245, 115, "Contributions", fmt(total_contributions), "last 12 months")
svg += card(425, 200, 245, 115, "Current Streak", f"{streak} days", "consecutive contribution days")
svg += card(690, 200, 245, 115, "Profile Updated", now_text, "generated automatically")
svg += svg_end()
...
for r, (x, y) in zip(featured, positions):
    name = r["name"]
    desc = (r.get("description") or "Public project by Debarko Ghosh")[:70]
...
# Language distribution based on the primary language of public repositories.
langs = {}
...
svg = svg_start(1100, 330, "🛠️ LANGUAGE MIX BY PUBLIC REPOSITORIES")
for i, (lang, amount) in enumerate(langs):
    y = 80 + i * 29
    width = int(700 * amount / total_lang)
    svg += f'''<text x="35" y="{y}" fill="#e8f0ff" font-family="Arial" font-size="13">{escape(lang)}</text><rect x="160" y="{y-13}" width="700" height="15" rx="7" fill="#101b2f"/><rect x="160" y="{y-13}" width="{width}" height="15" rx="7" fill="url(#g)"/><text x="885" y="{y}" fill="#7f91aa" font-family="Arial" font-size="13">{amount * 100 / total_lang:.1f}%</text>'''
svg += svg_end()
